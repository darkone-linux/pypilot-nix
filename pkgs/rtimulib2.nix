# RTIMULib2 — IMU sensor fusion, pypilot fork (seandepagnier).
#
# Packaged as the Python `RTIMU` binding, which is exactly what pypilot
# depends on. The binding vendors and compiles the C++ sources itself, so no
# separate libRTIMULib build/link step is required.
